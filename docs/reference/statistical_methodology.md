...
```
T_measured = WMO_round(T_true + ε)
WMO_round(x) = floor(x + 0.5)           # WMO 非对称 half-up，不是 banker's
ε ~ N(0, σ_instrument²)
σ_instrument ≈ 0.3°C ≈ 0.5°F
```

**⚠️ 重要**：`WMO_round` **不是** Python 内置 `round()`（banker's / round-half-to-even，`round(52.5)==52`，`round(-1.5)==-2`，**错误**），也不是 `np.round()`（同样 banker's，错误）。WU 与 NWS 按 WMO 标准使用 `floor(x + 0.5)` 实现非对称 half-up：52.5→53、-0.5→0、-1.5→-1、-2.5→-2。

对于预报值 T_forecast ± σ_forecast：

```
P(T_measured ∈ [a, b]) = ∫ P(WMO_round(T + ε) ∈ [a, b]) · f(T; T_forecast, σ_forecast) dT
```
...
def bin_probability(forecast_mean, forecast_std, bin_low, bin_high,
    ...
    # WMO 非对称 half-up：floor(x + 0.5)，不是 np.round() 的 banker's rounding
    T_measured = np.floor(T_true + np.random.normal(0, instrument_noise, n_samples) + 0.5)
    return np.mean((T_measured >= bin_low) & (T_measured <= bin_high))
```

这比简单的 `count(ensemble_member ∈ bin) / 51` 更准确，尤其在 bin 边界附近。**并且使用正确的 WMO 四舍五入规则很关键**——如果 Monte Carlo 用 `np.round` 而 WU 用 `floor(x+0.5)`，每个 bin 边界附近的概率质量分配都会系统性错位，在偶数整数 .5 处最严重（52.5、74.5）和负数（-1.5、-3.5）。`SettlementSemantics` 必须配置 `rounding_rule='wmo_half_up'`；`round_half_to_even` 是历史缺陷，不能恢复。
