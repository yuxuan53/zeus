...
| T6.4 D6 fee + time cost wired into HoldValue at live exit seam (minimal scope; phase2 defers correlation crowding) | closed_minimal | pending | 5 files (1 new src helper + 3 src edits + 1 new test file) + 2 config entries + 1 feature flag + 2 receipt/work_log. **Plan-premise corrections introduced**: (#18) src/execution/exit_triggers.py has ZERO production callers — real seam is Position.evaluate_exit ← cycle_runtime.py:630 inside src/state/** planning-lock zone. (#21) _buy_no_exit path bypassed HoldValue entirely (bare `shares*current_market_price <= shares*current_p_posterior` math); T6.4 brings it through the contract for parity with _buy_yes_exit. (#22) HoldValue.compute() accepted extra_costs dict per docstring but __post_init__ validator only checked `gross-fee-time`, leaving extras-passing callers with ValueError at construction — latent bug surfaced when T6.4 correlation_crowding test triggered; fix added trailing `extra_costs_total: float = 0.0` field with validator accounting. **Implementation**: (1) `config/settings.json` adds `exit.fee_rate=0.05` (matches polymarket_fee default) + `exit.daily_hurdle_rate=0.0001` (0.01%/day conservative hurdle) + `feature_flags.HOLD_VALUE_EXIT_COSTS=false` (default OFF for shadow validation). (2) `src/config.py` adds 3 getters (`exit_fee_rate`, `exit_daily_hurdle_rate`, `hold_value_exit_costs_enabled`). (3) `src/contracts/hold_value.py` adds `HoldValue.compute_with_exit_costs(shares, current_p_posterior, best_bid, hours_to_settlement, fee_rate, daily_hurdle_rate, correlation_crowding=0.0)` factory reusing `src.contracts.execution_price.polymarket_fee` for fee computation; time_cost = capital_locked × (hours/24) × daily_hurdle; hours=None or negative collapses time_cost to 0.0 soft default; correlation_crowding param reserved for T6.4-phase2 portfolio-context threading. (4) `src/state/portfolio.py` rewires 4 EV-gate sites: `_buy_yes_exit` L545 (Day0) + L598 (Layer-4 EV) + `_buy_no_exit` L684 (Day0, previously bypass) + L760 (Layer-4 EV, previously bypass). Flag-gated: OFF preserves pre-T6.4 zero-cost behavior; ON routes through compute_with_exit_costs. `_buy_yes_exit` signature extended with `hours_to_settlement: Optional[float] = None` (previously missing); 2 callers updated to thread `exit_context.hours_to_settlement`. (5) NEW `tests/test_hold_value_exit_costs.py` with 12 tests across 3 classes: ComputeWithExitCosts factory arithmetic (fee polymarket formula, time scales with hours+hurdle, hours-None-collapses-to-zero, negative-hours-defensive, correlation_crowding-default-is-noop, correlation_crowding-nonzero-is-recorded, net_value-invariant); FlagOffPreservesPreT64Semantics (zero-cost compute matches legacy shape); PortfolioExitIntegration (4 tests — flag OFF/ON × buy_yes/buy_no × applied_validations breadcrumb presence). **Regression**: touched scope 53P/0F/0xfail; broader 80P/1F (1 pre-existing vwmp unrelated). 15 hold_value-keyed tests all pass (dataclass field addition backwards-compatible with existing callers). **Feature flag flip protocol**: operator validates should_exit delta via T6.3-followup-1 replay harness (soft prerequisite) + live A/B on capital fraction (stronger signal); current OFF is the safety gate. **T6.4-phase2 tracked in T6_receipt.json** for correlation crowding via ExitContext portfolio-position threading (~6h; D6 full 1.0/1.0 immunity). Planning-lock GREEN (src/state/** + src/contracts/** in lock zones; `--plan-evidence` passes). | 2026-04-24 |
...
## P-1 Pre-P0 Post-Audit Cleanup — 2026-04-24

Date: 2026-04-24
Branch: data-improve
Task: P-1 pre-P0 post-audit cleanup from `POST_AUDIT_HANDOFF_2026-04-24.md` fixes 4.1.A-C.
Changed files:
...
Summary: Closed the in-flight P-1 items before forensic P0: city onboarding no longer writes provenance-empty settlement scaffold rows; the `settlements_non_null_metric` trigger has a pytest antibody; the unused `append_event_and_project` helper and DB re-export are removed so only the SAVEPOINT-safe `append_many_and_project` path remains; topology-doctor helper detection no longer advertises the deleted helper; harvester comments and test fixture now describe REOPEN-2 `(city, target_date, temperature_metric)` idempotency; the touched architecture-contract harness now includes `warning()` so the full state-kernel test slab can exercise the canonical dual-write skip path.
Verification:
- `.venv/bin/python -m pytest -q tests/test_settlements_unique_migration.py tests/test_harvester_dr33_live_enablement.py` -> `36 passed`.
- `.venv/bin/python -m pytest -q tests/test_dt1_savepoint_integration.py tests/test_db.py tests/test_architecture_contracts.py` -> `106 passed, 41 skipped`.
- `.venv/bin/python scripts/onboard_cities.py --dry-run --cities Auckland` -> completed; settlement scaffolding step logs a skip/no-op.
- `.venv/bin/python scripts/topology_doctor.py --planning-lock --changed-files scripts/onboard_cities.py scripts/topology_doctor_script_checks.py src/execution/harvester.py src/state/db.py src/state/ledger.py tests/test_architecture_contracts.py tests/test_harvester_dr33_live_enablement.py tests/test_settlements_unique_migration.py docs/operations/task_2026-04-23_midstream_remediation/work_log.md docs/operations/task_2026-04-23_midstream_remediation/receipt.json --plan-evidence docs/operations/task_2026-04-23_midstream_remediation/POST_AUDIT_HANDOFF_2026-04-24.md --json` -> `{ok: true, issues: []}`.
- `rg -n "\bappend_event_and_project\b" src scripts tests` -> only absence-assertion tests remain.
...
1. `docs/to-do-list/zeus_midstream_fix_plan_2026-04-23.md` — added "Closure snapshot" section at top (W1–W4 closure table + out-of-plan T6.4-phase2 + Day0-canonical-event highlights + all 24 plan-premise corrections in one table); per-slice closure status for W1–W4 with commit hashes; "Wave 5 remaining blockers" section with per-slice concrete blocker + unblock condition; "Out-of-plan deferrals surfaced during execution" section listing 7 tracked items (T6.3-followup-1 corpus audit, T6.4 pre-flag-flip checklist, Day0-canonical production migration, T1.c-followup L1536/L1569 operator decision, T1.d NC-12 L70 Phase-7 wait, T6.4 MED-5 buy_no kwarg, Day0TemporalContext fixture promotion, T2.g sibling stub cleanup) each with scope + blocker + estimated size.
2. `docs/operations/known_gaps.md` — promoted D5 MITIGATED → CLOSED (T6.3 Option C sibling_snapshot + imputation_source provenance wiring), D6 MITIGATED → CLOSED (T6.4 minimal + phase2 correlation crowding via ExitContext portfolio threading, feature-flag + rate-default-zero safety), added Day0 canonical event emission entry documenting build_day0_window_entered_canonical_write + DAY0_WINDOW_ENTERED event type + legacy-DB migration.
3. `docs/operations/current_state.md` — restructured "Active program" section: W1–W4 100% closed + out-of-plan T6.4-phase2 and Day0-canonical highlighted; W5 blockers enumerated as non-code-addressable (T6.1/T6.2/N1.1 corpus, T4.2-Phase2 7-day window, T3.4 upstream); out-of-plan deferrals list with pointers to fix-plan workbook section. **Architecture/** manifests NOT updated: per peer-slice precedent (T5 test files test_tick_size/test_realized_fill/test_neg_risk_passthrough were not registered in test_topology.yaml), new test files test_hold_value_exit_costs.py + test_vig_treatment_provenance.py follow same convention — map-maintenance precommit warns advisory-level, non-blocking. D5/D6 CLOSED status lives in known_gaps.md per existing convention (architecture/invariants.yaml does not track D-series). | 2026-04-24 |

## W4 closure (2026-04-24, team-lead session)

| W4 closure — T2.g real-Day0Router fixture + N1.2 retroactive accounting | closed | pending | **T2.g**: 1 test file edit (tests/test_fdr.py) + T2_receipt update. Removes xfail(strict=False) marker at L1109-1123; replaces SimpleNamespace stub with real Day0TemporalContext + SolarDay fixture (Dallas/2026-04-12/CDT DST-active; sunrise ≈ 07:00 CDT / sunset ≈ 19:45 CDT, peak_hour=15, post_peak_confidence=0.4, daylight_progress=0.5). Real Day0Router.route dispatch path now exercised end-to-end; terminal assertion decisions[0].rejection_stage == 'SIZING_TOO_SMALL' matches the monkeypatched T2.d variant. **Category immunity**: future Day0TemporalContext schema drift (new required field, renamed field, dropped attribute) now fails LOUD via this test instead of silently xfail-passing. **N1.2 (plan-premise correction #23)**: persistence_anomaly at src/engine/monitor_refresh.py:437-510 already implements plan L124 spec — 3-day window (L469), min_samples ≥ 30 (L501), confidence-scaled discount (L503 `min(0.30, 0.10 + 0.20 * (n - 30) / 70.0)`), LOW-metric gate (L458-464). Both persistence_anomaly entries in docs/operations/known_gaps.md:104,111 already marked [FIXED] since 2026-03-31. Test coverage: tests/test_phase10c_dt_seam_followup.py + tests/test_instrument_invariants.py::test_persistence_discount_requires_adequate_samples both pass. N1.2 is retroactive plan-row accounting — no new code. **W4 effective status**: 5/5 shipped or retroactively closed. T5.d (782d2af), T6.4 (96fd850 + 6b4455f + 4edd4c8), T2.g (this commit), N1.2 (retroactive), T3.4-observe upstream-blocked. Regression: test_fdr.py 19/19 + persistence 16/16 pass; zero new failures. Planning-lock GREEN (tests/* + docs/* only). | 2026-04-24 |
