...
| 15a | Module manifest → __init__.py registries | ~~DO IT~~ **AUDIT-AND-KEEP per Phase 3 erratum** | ~~Python registries for active packages~~ Phase 3 audit: 21/25 = 84% modules have NO __init__.py runtime registry to absorb hand-curated metadata (intentional YAML curation). 4 HYBRID candidates (contracts, risk_allocator, strategy, types) deferred to Phase 3.5. | Audit script `scripts/module_manifest_audit.py` 216L; KEEP_AS_YAML majority. See round2_verdict §9.3 erratum |
