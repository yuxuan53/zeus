...
| Lifecycle terminal vocab | `src/state/portfolio.py:962` `{settled, voided, admin_closed, quarantined}` (canonical) + `cycle_runner.py:54` agreeing mirror + `cycle_runner.py:341` `{settled, voided, admin_closed, economically_closed}` (DISAGREES) + AGENTS.md §1 ("Terminal states: voided, quarantined, admin_closed" — incomplete, missing `settled`) + `LEGAL_LIFECYCLE_FOLDS` ground truth | **RESOLVED post-rebase 2026-04-26**: ground truth = `LEGAL_LIFECYCLE_FOLDS`; canonical terminal set = `{settled, voided, admin_closed, quarantined}` (4 phases whose fold equals `{phase}`). `cycle_runner.py:341` has wrong members; AGENTS.md §1 prose missed `settled`. | **CONFIRMED CANONICAL — slice B1 fixes site 3 (semantic) and updates AGENTS.md (doc fix)** |

**Provenance resolution (post-rebase 2026-04-26):** Reading the full `LEGAL_LIFECYCLE_FOLDS` table at `src/state/lifecycle_manager.py:79-83`, four phases satisfy the terminal definition (fold = `{phase}` only): `SETTLED`, `VOIDED`, `QUARANTINED`, `ADMIN_CLOSED`. `ECONOMICALLY_CLOSED` does NOT satisfy this (folds to `{ECONOMICALLY_CLOSED, SETTLED, VOIDED}`). Code (folds + portfolio.py + cycle_runner mirror) wins; AGENTS.md prose is incomplete (missing `settled`); `cycle_runner.py:341` literal is wrong. Slice B1 derives `TERMINAL_STATES` programmatically from the folds dict, eliminating the possibility of future hardcoded-set drift.
...
6. **~~Whether `quarantined` should be in code's terminal set or stay separate.~~** RESOLVED post-rebase 2026-04-26: `LEGAL_LIFECYCLE_FOLDS` proves `QUARANTINED` IS terminal (fold = `{QUARANTINED}` only). Slice B1 derives canonical set programmatically from folds; `cycle_runner.py:341` gets semantic fix; AGENTS.md §1 prose gets a small doc patch to add `settled` to the terminal list.
