...
7. **Settlements table rebuilt FROM SCRATCH (delete-and-reinsert) with rows carrying full provenance chain, decision_time_snapshot_id, source-correct observation**. Target final count: 1,562 rows total (matching original bulk-batch set of (city, target_date) pairs), split between `VERIFIED` (observation-derived, audit-passed) and `QUARANTINED` (explicit reason in provenance_json). NO rows carried over as UPDATEs from the existing corrupted rowids. Exact VERIFIED count pending P-C audit outcome (range ~1,430-1,505 depending on WU audit per-city results).
8. **Harvester atomicity refactor passes all test scenarios with feature flag** (P-H closed)
