...
- Operator decision recorded: `opening_inertia` remains unchanged for now because the current `edge_compression` evidence is too thin to justify a gate. The warning is preserved as an analytical class, but the current sample/time profile is now treated as insufficient evidence for policy action.
