...
| P2-H | P1 | Forecast-layer Phase-1 de-hardcode: Day0 backbone, lead-continuous mean/sigma, heteroscedastic sigma | forecast lane | IN_PROGRESS | Forecast-layer work is now in its first behavior-changing stage: uncertainty policy is centralized behind `src/signal/forecast_uncertainty.py`; day0 sigma + observation-weight + residual-blend policy are extracted; the analysis sigma seam carries `lead_days` and `ensemble_spread`; and analysis bootstrap sigma now inflates continuously from 1.0x at day0 to 1.2x by day6+ instead of staying flat across all non-day0 leads. |
