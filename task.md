...
| P2-H | P1 | Forecast-layer Phase-1 de-hardcode: Day0 backbone, lead-continuous mean/sigma, heteroscedastic sigma | forecast lane | IN_PROGRESS | First low-blast-radius seam is landed: forecast uncertainty policy is now extracted behind `src/signal/forecast_uncertainty.py` and consumed by day0 + market-analysis paths without changing current outputs. Next work in this lane is behavior-changing forecast de-hardcode, not seam extraction. |
