...
| T6 | P0 | Day0 terminal phase for all positions | Agent A | IN_PROGRESS | T3 | `src/engine/monitor_refresh.py`, `src/state/portfolio.py`, `src/engine/cycle_runtime.py` | phase transition + runtime wiring | lifecycle/day0 tests |
...
| T10 | P1 | ExecutionReport / fill telemetry / order attempt events | Agent B | IN_PROGRESS | T2 | `src/execution/executor.py`, `src/execution/exit_lifecycle.py`, runtime writers | entry-path execution telemetry landed; exit-lifecycle order events still missing | unit tests |
...
| T17 | P1 | position_events reader/owner contract | Agent B / Agent C | TODO | T2,T9 | `src/state/db.py`, downstream readers | decide how replay/tracker/risk consumers use `position_events` vs `decision_log`/`chronicle` | contract review |
| T18 | P1 | exit_lifecycle event emission | Agent B | TODO | T10 | `src/execution/exit_lifecycle.py`, runtime writers | emit sell placement/retry/fill-check events into `position_events` | unit tests |
...
| T13 | P1 | RiskGuard input contract upgrade | Agent C | BLOCKED | T9,T12 | `src/riskguard/riskguard.py`, `src/riskguard/metrics.py` | strategy/execution-aware authoritative inputs | riskguard tests |
