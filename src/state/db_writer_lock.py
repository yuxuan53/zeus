...
SQLITE_CONNECT_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        "scripts/replay_day0_ask_repricing_veto.py",  # read_only_ro_uri: opens the trades DB via file:...?mode=ro uri with PRAGMA query_only=ON; SELECT-only over venue_commands + venue_trade_facts + position_current + executable_market_snapshots + payout_observations; writes NOTHING (prints to stdout); out-of-sample replay of the Day0 held-ask repricing veto (window-A admission study 2026-09-05)
